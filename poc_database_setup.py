...
class PoCDatabaseSetup:
    def __init__(self, db_path="poc_mastery.db"):
    ...
    def insert_sample_items(self):
        ...
        self.conn.commit()
        print(f"샘플 아이템 {len(sample_items)}개 삽입 완료")
    
    def insert_sample_situations(self):
        """샘플 상황 데이터 삽입"""
        cursor = self.conn.cursor()
        ...
        self.conn.commit()
        print(f"샘플 상황 {len(sample_situations)}개 삽입 완료")
    ...
    def verify_data(self):
        ...
        situation_count = cursor.fetchone()[0]
        print(f"상황 개수: {situation_count}개")
        
        # 샘플 데이터 출력
        print("\n=== 아이템 샘플 ===")
        cursor.execute("SELECT * FROM items LIMIT 5")
        for row in cursor.fetchall():
            print(f"ID: {row['item_id']}, 이름: {row['item_name']}, 타입: {row['item_type']}, 스탯: {row['base_stats']}")
        
        print("\n=== 상황 샘플 ===")
        cursor.execute("SELECT * FROM situations LIMIT 5")
        for row in cursor.fetchall():
            print(f"ID: {row['situation_id']}, 이름: {row['situation_name']}, 추천아이템: {row['recommended_items']}")
...
def main():
    """메인 실행 함수"""
    print("=== PoC 데이터베이스 설정 시작 ===")
    
    # 데이터베이스 설정
    db_setup = PoCDatabaseSetup()
    db_setup.connect()
    ...
    try:
        ...
        # 샘플 데이터 삽입
        db_setup.insert_sample_items()
        db_setup.insert_sample_situations()
        
        # 데이터 검증
        db_setup.verify_data()
        
        print("\n=== PoC 데이터베이스 설정 완료 ===")
        print("이제 LLM 연동 및 웹 인터페이스 구현을 진행할 수 있습니다!")
